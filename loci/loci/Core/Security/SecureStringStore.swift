...
public enum SecureStoreError: LocalizedError, Equatable {
  ...
  public var errorDescription: String? {
    switch self {
    case .readFailed(let status): return "Failed to read from Keychain (OSStatus: \(status))."
    case .writeFailed(let status): return "Failed to write to Keychain (OSStatus: \(status))."
    case .deleteFailed(let status): return "Failed to delete from Keychain (OSStatus: \(status))."
    case .invalidEncoding: return "Data in Keychain could not be encoded as a UTF-8 string."
    }
  }
}
...
public nonisolated final class KeychainStringStore: SecureStringStoring, @unchecked Sendable {
  ...
  public func string(for key: String) throws -> String? {
    ...
    guard let data = result as? Data, let value = String(data: data, encoding: .utf8) else { throw SecureStoreError.invalidEncoding }
    return value
  }
  ...
  public func removeValue(for key: String) throws {
    ...
    guard status == errSecSuccess || status == errSecItemNotFound else { throw SecureStoreError.deleteFailed(status) }
  }
}
