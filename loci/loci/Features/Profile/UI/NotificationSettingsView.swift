...
public struct NotificationSettingsView: View {
      ...
      Section("Push Notifications") {
        ...
        if notificationManager.authorizationStatus == .notDetermined {
          Button {
            ...
            HStack {
              if isRequesting { ProgressView().padding(.trailing, 4) }
              Text("Enable Notifications").fontWeight(.semibold)
            }
  ...
  private var statusDescription: String {
    ...
    case .authorized, .provisional, .ephemeral: return "Push notifications are active and ready."
    case .denied: return "Notifications are disabled in iOS Settings."
    case .notDetermined: return "Permissions have not been requested yet."
