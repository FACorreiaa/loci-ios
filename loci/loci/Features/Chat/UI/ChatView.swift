  ...
  public var body: some View {
    NavigationStack {
      VStack(spacing: 0) {
            ...
            LazyVStack(spacing: 12) {
              ForEach(messages) { msg in
                HStack {
                  if msg.role == "user" {
                    Spacer()
                    Text(msg.text).padding(14).background(Color.lociCoral).foregroundColor(.white).cornerRadius(LociTheme.cornerRadius).frame(
                      maxWidth: 280,
                      alignment: .trailing
                    )
                  } else {
                    HStack(alignment: .top, spacing: 8) {
                      Image("LociMascot").resizable().scaledToFit().frame(width: 32, height: 32).padding(.top, 4)

                      Text(msg.text).padding(14).background(Color.lociCard).foregroundColor(.lociInk).cornerRadius(LociTheme.cornerRadius).overlay(
        ...
        // Input Bar
        HStack(spacing: 10) {
          TextField("Ask anything about your destination...", text: $inputText).padding(.horizontal, 14).padding(.vertical, 10).background(
            Color.lociCard
          ).cornerRadius(20).overlay(RoundedRectangle(cornerRadius: 20).stroke(Color.lociBorder.opacity(0.6), lineWidth: LociTheme.borderWidth))
            .onSubmit { sendMessage() }

          Button(action: sendMessage) {
            Image(systemName: "arrow.up.circle.fill").font(.system(size: 32)).foregroundColor(
              inputText.trimmingCharacters(in: .whitespaces).isEmpty ? .lociInk.opacity(0.3) : .lociCoral
            )
          }.disabled(inputText.trimmingCharacters(in: .whitespaces).isEmpty || isSending)
        }.padding(.horizontal, 16).padding(.vertical, 10).background(Color.lociPaper)
      }.background(Color.lociPaper.ignoresSafeArea()).navigationTitle("AI Assistant")
    }
  }
