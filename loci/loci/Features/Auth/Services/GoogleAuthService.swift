...
@MainActor
public final class GoogleAuthService: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    public init(
        client: Loci_CustomAuth_CustomAuthServiceClient? = nil,
    ...
    public func signInWithGoogle() async throws -> Loci_CustomAuth_OAuthCallbackResponse {
        ...
        if let err = res.error {
            throw APIError.custom(err.message ?? "Failed to initialize Google authentication.")
        }
        guard let message = res.message, let authURL = URL(string: message.authURL) else {
        ...
        if let err = callbackRes.error {
            throw APIError.custom(err.message ?? "Google authentication failed.")
        }
        guard let callbackMsg = callbackRes.message else {
            throw APIError.invalidResponse
        }
