...
public final class AuthService {
    ...
    public init(
        client: Loci_Auth_AuthServiceClient? = nil,
    ...
    public func login(email: String, password: String) async throws -> Loci_Auth_LoginResponse {
        ...
        if let err = res.error {
            throw APIError.custom(err.message ?? "Authentication failed.")
        }
    ...
    public func register(email: String, username: String, password: String) async throws -> Loci_Common_Response {
        ...
        if let err = res.error {
            throw APIError.custom(err.message ?? "Registration failed.")
        }
        guard let message = res.message else {
            throw APIError.invalidResponse
        }
    ...
    public func verifyMFA(mfaToken: String, code: String, recoveryCode: String? = nil) async throws -> Loci_Auth_LoginResponse {
        ...
        if let recoveryCode, !recoveryCode.isEmpty {
            req.recoveryCode = recoveryCode
        } else {
            req.code = code
        }
        ...
        if let err = res.error {
            throw APIError.custom(err.message ?? "MFA verification failed.")
        }
        guard let message = res.message else {
            throw APIError.invalidResponse
        }
    ...
    public func forgotPassword(email: String) async throws {
        ...
        if let err = res.error {
            throw APIError.custom(err.message ?? "Password reset failed.")
        }
    }
