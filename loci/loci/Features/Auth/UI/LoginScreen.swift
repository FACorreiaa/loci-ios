        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    HStack(spacing: 0) {
                        Button {
                            ...
                            Text("Sign In")
                                ...
                                .foregroundColor(viewModel.isSignup ? .lociInk.opacity(0.5) : .lociInk)
                        ...
                        Button {
                            ...
                            Text("Create Account")
                                .font(.subheadline.weight(.semibold))
                                .foregroundColor(viewModel.isSignup ? .lociInk : .lociInk.opacity(0.5))
                                .frame(maxWidth: .infinity)
                                .padding(.vertical, 10)
                                .background(
                                    ZStack {
                    ...
                    .padding(4)
                    .background(Color.lociMuted.opacity(0.5))
                    .cornerRadius(LociTheme.cornerRadius + 2)
                    ...
                    HStack(spacing: 16) {
                        Rectangle()
                            .fill(Color.lociBorder.opacity(0.8))
                            .frame(height: 1)
                        Text("or")
                            .font(.footnote)
                    ...
                    Button {
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: LociTheme.cornerRadius)
                                .stroke(Color.lociBorder.opacity(0.8), lineWidth: LociTheme.borderWidth)
                        )
                    }
            ...
            .sheet(isPresented: $viewModel.showMFAModal) {
                if let token = viewModel.pendingMFAToken {
                    MFACodeSheet(
                        mfaToken: token,
                        onSubmit: { code, recoveryCode in
                            viewModel.handleMFASubmit(code: code, recoveryCode: recoveryCode)
                        },
                        onCancel: {
