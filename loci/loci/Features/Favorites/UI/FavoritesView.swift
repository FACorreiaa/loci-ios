...
public struct FavoritesView: View {
    ...
    public init(client: Loci_Favorites_V1_FavoritesServiceClient = Loci_Favorites_V1_FavoritesServiceClient(client: ConnectTransport.shared.protocolClient)) {
        self.client = client
    }
                ...
                if isLoading {
                    ...
                    VStack(spacing: 16) {
                        Image(systemName: "bookmark.circle.fill")
                            .font(.system(size: 64))
                            .foregroundColor(.lociCoral.opacity(0.8))

                        Text("No Saved Places Yet")
                            .font(.title3.weight(.bold))
                            .foregroundColor(.lociInk)

                        Text("Tap the bookmark icon on any destination or restaurant to save it to your collection.")
                            .font(.subheadline)
                            .foregroundColor(.lociInk.opacity(0.7))
