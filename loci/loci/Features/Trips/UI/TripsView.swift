            ...
            VStack {
                if isLoading {
                    ...
                    VStack(spacing: 16) {
                        ...
                        Text("Start a conversation in Chat or discover places to build your first tailored itinerary.")
                            .font(.subheadline)
                            .foregroundColor(.lociInk.opacity(0.7))
                    ...
                    List(trips, id: \.id) { trip in
                        VStack(alignment: .leading, spacing: 6) {
                            Text(trip.title.isEmpty ? (trip.cityName.isEmpty ? "Untitled Trip" : "Trip to \(trip.cityName)") : trip.title)
                                .font(.headline.weight(.semibold))
                                .foregroundColor(.lociInk)

                            HStack {
                                if !trip.cityName.isEmpty {
                                    Label(trip.cityName, systemImage: "mappin.circle.fill")
                                        .font(.caption)
                                        .foregroundColor(.lociCoral)
                                }
                                Spacer()
